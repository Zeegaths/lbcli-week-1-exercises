...
# WRITE YOUR SOLUTION BELOW:
LEGACY_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet -named getnewaddress address_type=legacy)
check_cmd "Legacy address generation"

P2SH_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet -named getnewaddress address_type=p2sh-segwit)
check_cmd "P2SH address generation"

SEGWIT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet -named getnewaddress address_type=bech32)
check_cmd "SegWit address generation"

TAPROOT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet -named getnewaddress address_type=bech32m)
...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest validateaddress $P2SH_ADDR | grep -o '"isvalid": true' | grep -o 'true')
...
# WRITE YOUR SOLUTION BELOW:
NEW_TAPROOT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet -named getnewaddress address_type=bech32m)
...
# WRITE YOUR SOLUTION BELOW:
DESC=$(echo "$ADDR_INFO" | grep -o '"desc" *: *"[^"]*"' | cut -d':' -f2- | tr -d ' "')
INTERNAL_KEY=$(echo "$DESC" | sed -n 's/^tr(\([^)]*\)).*$/\1/p')
